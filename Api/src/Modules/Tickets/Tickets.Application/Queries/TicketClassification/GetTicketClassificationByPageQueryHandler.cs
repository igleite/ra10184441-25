...
public class GetTicketClassificationByPageQueryHandler : IQueryHandler<GetTicketClassificationByPageQuery, PageDto<TicketClassificationDto>>
{
    private readonly IUnitOfWork _unitOfWork;

    public GetTicketClassificationByPageQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<PageDto<TicketClassificationDto>> Handle(GetTicketClassificationByPageQuery request, CancellationToken cancellationToken)
    {
...
{_unitOfWork.SdpDpNew.Offset()}");
        ...
        var data = await multi.ReadAsync<TicketClassificationDto>();

        if (data is null || !data.Any())
            throw AppException.NotFound($"Nenhuma classificação de ticket encontrada!");

        return new PageDto<TicketClassificationDto>
        {
