...
public class UpdateStatusReasonCommandHandler : ICommandHandler<UpdateStatusReasonCommand, StatusReasonDto>
{
    private readonly IDateTimeProvider _dateTimeProvider;
    private readonly IStatusReasonRepository _repository;

    public UpdateStatusReasonCommandHandler(IDateTimeProvider dateTimeProvider, IStatusReasonRepository repository)
    {
        _dateTimeProvider = dateTimeProvider;
        _repository = repository;
    }

    public async Task<StatusReasonDto> Handle(UpdateStatusReasonCommand request, CancellationToken cancellationToken)
    {
        ...
        if (statusReason is null)
            throw AppException.NotFound($"O motivo de status {request.Name} não existe!");
        ...
        var success = await _repository.UpdateAsync(statusReason);
        if (!success)
            throw AppException.BadRequest($"O motivo de status {request.Name} não foi atualizado!");
